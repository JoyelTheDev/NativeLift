...
class LifterOptions:
    ...
    # ---- Stack balancing ----
    #: Apply the :class:`~.stack_tracker.StackTracker` post-pass that
    #: inserts corrective ACONST_NULL / ICONST_0 / … placeholder pushes
    #: before any instruction that would underflow the operand stack.
    #: Dramatically reduces the fraction of methods that ``class-rebuilder``
    #: downgrades to stubs due to ASM ``COMPUTE_FRAMES`` rejection.
    use_stack_tracker: bool = True
