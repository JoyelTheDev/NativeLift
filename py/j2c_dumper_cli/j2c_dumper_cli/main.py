...
def recover(
    ...
    run_cmd: Optional[list[str]] = typer.Option(None, "--run-cmd",
                                                 help="Command(s) to execute the jar for dynamic trace. "
                                                      "Repeat for multiple runs; traces are merged for better branch coverage."),
    ...
    report: Optional[Path] = typer.Option(None, "--report", help="Write an HTML recovery report to this path after rebuild."),
    validate_schemas: bool = typer.Option(True, "--validate-schemas/--no-validate-schemas",
                                          help="Enforce schemaVersion on all pipeline artifacts."),
):
    """One-shot orchestration: parse → introspect → merge → trace → recover → rebuild.

    Supply --run-cmd multiple times to run the target in different modes and merge
    all traces for better branch coverage.
    """
    if workdir is None:
    ...
    if not no_dynamic and run_cmd:
        ...
        from .schema_validate import validate_artifact, SchemaError
        console.rule("[4/6] dynamic-trace")
        per_run_traces: list[Path] = []
        for idx, cmd in enumerate(run_cmd):
            run_trace = workdir / f"trace-run{idx}.jsonl"
            _run_dynamic_trace(cmd, run_trace)
            per_run_traces.append(run_trace)
        if len(per_run_traces) > 1:
            console.log(f"[cyan]merging {len(per_run_traces)} trace files…[/]")
            n = merge_traces(per_run_traces, trace_jsonl)
            console.log(f"[cyan]merged → {n} events[/]")
        else:
            import shutil as _shutil
            _shutil.copy2(per_run_traces[0], trace_jsonl)
        # Schema-validate the merged trace (first line only, for speed)
        if validate_schemas:
            import json as _json
            first_line = trace_jsonl.read_text(encoding="utf-8").split("\n")[0]
            if first_line:
                ev = _json.loads(first_line)
                if "schemaVersion" not in ev:
                    ev["schemaVersion"] = 1
                    console.log("[yellow]trace events have no schemaVersion; skipping version check[/]")
    ...
    # Schema-validate pipeline artifacts before rebuild
    if validate_schemas:
        from .schema_validate import validate_artifact, SchemaError
        for artifact_path, kind in [
            ...
            if artifact_path.exists():
                try:
                    validate_artifact(artifact_path, kind)
                except SchemaError as exc:
                    console.print(f"[bold yellow]schema warning:[/] {exc}")
    _run_rebuild(jar, recovered_dir, output, manifest_json)
    console.print(f"[bold green]done:[/] {output}")

    # Optional HTML recovery report
    if report:
        from .report import generate_report
        console.log(f"[cyan]generating recovery report → {report}[/]")
        generate_report(manifest_json, recovered_dir, report)
